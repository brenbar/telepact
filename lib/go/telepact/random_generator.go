...
// NextIntWithCeiling generates a random integer between 0 and ceiling
...
func (rg *RandomGenerator) NextBytes() []byte {
	bytes := make([]byte, 4)
	binary.BigEndian.PutUint32(bytes, uint32(rg.NextInt()))
	return bytes
}
...
// NextDouble generates a random float64
